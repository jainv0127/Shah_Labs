...
clusters <- data.frame(GeneID=names(clarax$clustering),Cluster=clarax$clustering,stringsAsFactors=F)
write.table(clusters,file.path(output,species),sep="\t",col.names=T,row.names=F,quote=F)

pdf(file.path(output,paste0(species,"_",k,".pdf")))
