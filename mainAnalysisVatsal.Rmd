...
```
```{r}
gc.values <- tree.data$GC
names(gc.values) <- tree.data$Species
dtgcn.values <- tree.data$Diversity.tGCN
names(dtgcn.values) <- tree.data$Species
y.pic <- pic(x=dtgcn.values, phy=tree.filt)
x.pic <- pic(x=gc.values,phy=tree.filt)

cor.test(x.pic,y.pic,method="spearman")
```
...
```{r}
em.values <- tree.data$Median.eM
names(em.values) <- tree.data$Species
dtgcn.values <- tree.data$Diversity.tGCN
names(dtgcn.values) <- tree.data$Species
x.pic <- pic(x=dtgcn.values, phy=tree.filt)
y.pic <- pic(x=em.values,phy=tree.filt)
...
```{r}
# use data=tgcn.summary.codon.order.yes not tree 
...
gc.values <- tgcn.summary.codon.order.yes$GC
names(gc.values) <- tgcn.summary.codon.order.yes$Species
em.values <- tgcn.summary.codon.order.yes$Diversity.tGCN
names(dtgcn.values) <- tgcn.summary.codon.order.yes$Species
x.pic <- pic(x=gc.values, phy=tree.filt)
y.pic <- pic(x=em.values,phy=tree.filt)
...
```{r}
#use tgcn.summary.codon.order.no not tree data
...
```{r}
gc.values <- tgcn.summary.codon.order.no$GC
names(gc.values) <- tgcn.summary.codon.order.no$Species
em.values <- tgcn.summary.codon.order.no$Diversity.tGCN
names(dtgcn.values) <- tgcn.summary.codon.order.no$Species
x.pic <- pic(x=gc.values, phy=tree.filt)
y.pic <- pic(x=em.values,phy=tree.filt)
